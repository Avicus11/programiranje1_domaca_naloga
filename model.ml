...
let string_of_element = Int.to_string
...
let string_of_cell = Int.to_string
...
let get_box (grid : 'a grid) (box_ind : int) = 
  let i_row_ind = (box_ind / 3) * 3 in
  let i_col_ind = (box_ind mod 3) * 3 in
  Array.init 9 (fun n -> grid.(i_row_ind + (n / 3)).(i_col_ind + (n mod 3)))
...
let rec valid (array : int array) : bool =
  let length = Array.length array in
  if length < 2 then true 
  else
    let head = array.(0) and
    tail = Array.sub array 1 (length - 1) in
    if Array.exists (fun x -> x = head) tail then false else valid tail

let is_valid_sudoku (grid : solution) : bool = 
  (rows grid @ columns grid @ boxes grid) |> List.for_all valid

let is_subsudoku (problem : problem) (solution : solution) : bool = 
  let problem_grid = problem.initial_grid in
  let fun_comp i j element acc = 
    match element with 
    | None -> true && acc
    | Some y -> (solution.(i).(j) = y) && acc 
  in 
  foldi_grid fun_comp problem_grid true  

let is_valid_solution (problem : problem) (solution : solution) : bool =
  is_subsudoku problem solution && is_valid_sudoku solution 